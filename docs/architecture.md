...
| 自动回归测试 | API、检索、生成、评测与重排序模块 | 60 passed |

第 11～20 天继续补充了来源校验、检索诊断、稳态性能基准、`/ready` 与 GitHub Actions CI。第 21～30 天再增加了语料指纹、索引新鲜度检测、资料质量扫描、提示词安全边界和发布质量门禁。当前自动回归测试为 **60 passed**；在本机以 16 道困难题、重复 2 次测得混合检索稳态中位延迟为 **148.135 ms**，P95 为 **161.930 ms**（不含 DeepSeek）。
